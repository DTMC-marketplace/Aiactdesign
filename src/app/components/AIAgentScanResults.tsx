...
interface AIAgentScanResultsProps {
  toolName: string;
  taskName: string;
  article: string;
  onBack: () => void;
...
export function AIAgentScanResults({
  toolName,
  taskName,
  article,
  onBack,
}: AIAgentScanResultsProps) {
  ...
  const renderQuestion = (question: AssessmentQuestion) => {
    const hasAIDetection = question.aiDetectedValue !== undefined;

    if (question.type === 'checkbox' && question.options) {
      return (
        <div key={question.id} className="bg-white rounded-lg border border-[#F0F1F2] shadow-sm p-6 mb-6">
          {/* AI Detection Banner */}
          {hasAIDetection && Array.isArray(question.aiDetectedValue) && (
            <div className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg bg-[#F13D30] flex items-center justify-center">
                ...
                <div>
                  ...
                  <p className="font-['Montserrat',sans-serif] font-normal text-sm text-[#565F6C]">
                    Our AI agents have pre-filled these values with {question.aiConfidence} confidence
                  </p>
                </div>
              </div>

              <div className="space-y-3">
                {question.aiDetectedValue.map((value: string, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 p-4 bg-[#D1FAE5] border border-[#065F46] rounded-lg">
          ...
          <div className="space-y-3">
            {question.options.map((option, idx) => {
              const isChecked = hasAIDetection && Array.isArray(question.aiDetectedValue)
                ? question.aiDetectedValue.includes(option)
    ...
    if (question.type === 'radio' && question.options) {
      return (
        <div key={question.id} className="bg-white rounded-lg border border-[#F0F1F2] shadow-sm p-6 mb-6">
          {hasAIDetection && (
            <div className="mb-4 p-3 bg-[#DBEAFE] border border-[#1E40AF] rounded-lg flex items-center gap-2">
              <CheckCircle className="w-5 h-5 text-[#1E40AF] flex-shrink-0" />
              <span className="font-['Montserrat',sans-serif] font-medium text-sm text-[#1E40AF]">
                AI suggested: {question.aiDetectedValue} ({question.aiConfidence} confidence)
              </span>
            </div>
          )}
          ...
          <div className="space-y-3">
            {question.options.map((option, idx) => {
              const isSelected = hasAIDetection 
                ? question.aiDetectedValue === option
    ...
    <div className="flex-1 flex flex-col h-screen overflow-hidden bg-[#FAFBFC]">
      ...
      <div className="bg-white border-b border-[#F0F1F2] px-8 py-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button className="px-6 py-2.5 bg-white border-2 border-[#F13D30] text-[#F13D30] rounded-lg font-['Montserrat',sans-serif] font-bold text-sm hover:bg-[#FEEDEC] transition-colors flex items-center gap-2">
        ...
        <div className="max-w-[1000px] mx-auto px-8 py-8">
          ...
          <div className="flex items-center justify-between mt-8">
            <button
              ...
              className="px-6 py-2.5 bg-white border-2 border-[#E5E7EB] text-[#565F6C] rounded-lg font-['Montserrat',sans-serif] font-bold text-sm hover:bg-[#F9FAFB] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Previous
            </button>
            ...
            <button
              ...
              className="px-6 py-2.5 bg-[#F13D30] text-white rounded-lg font-['Montserrat',sans-serif] font-bold text-sm hover:bg-[#DC180A] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
